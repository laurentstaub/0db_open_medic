...
FILE_PATH="/Users/laurentstaub4/documents/00_open_medic/data/processed/csv_data/OPEN_MEDIC_2017.csv"
...
echo -e "\nCount of columns in first data row (may fail with encoding issues):"
...
head -n 1000 "$FILE_PATH" | cut -d';' -f13,15,16 | grep -v "^[0-9]\+;[0-9]\+;[0-9]\+$" | head -n 5

echo -e "\nChecking for French number format (comma as decimal separator):"
head -n 1000 "$FILE_PATH" | cut -d';' -f20,21 | grep "," | head -n 5
